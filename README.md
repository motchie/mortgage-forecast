...
| 残高と月返済額の推移 | 各シナリオの残高と月返済額を時系列で比較します。設定によりグラフと表、または表だけを表示します。 |
| 125%上限が発動した見直し | 理論上の返済額が上限を超えた見直しについて、理論額、上限額、実際に採用した額を表示します。 |
| 一定金利の感応度分析 | 一定の金利が満期まで続くと仮定し、金利水準ごとの返済リスクを機械的に比較します。 |
...
| Base / Higher / Stress | 利用者が設定した将来金利の仮定です。名称は比較用であり、発生確率や危険度を表しません。 |
...
| 残高推移 / 月返済額推移 | Current / Base / Higher / Stressごとの年末残高と通常月返済額の変化です。グラフの縦軸は万円単位で、表では円単位の値を確認できます。 |
